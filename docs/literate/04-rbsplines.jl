...
# fig = MK.series(t, Bs', color = :Set1,
#     axis = (title =  "(a) B-splines", xgridvisible = false));
# MK.vlines!(range(boundaryknots(bs)), color = :gray, linestyle = :dash, linewidth = 1.2);
...
fig = MK.Figure(resolution = (800, 500))
MK.Axis(fig[1,1], title =  "(b) B-splines Integral", xgridvisible = false)
MK.vlines!(range(boundaryknots(bs)), color = :gray, linestyle = :dash, linewidth = 1.5);
[MK.lines!(t, Bi[:,i], label = "$i") for i = 1:size(Bs, 2)]
MK.save("makie.pdf", fig) #src
