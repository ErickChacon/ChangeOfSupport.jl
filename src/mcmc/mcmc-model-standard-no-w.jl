function sample_model_standard_now(y, x, Bvx, Bvy, Pv, id; binit = nothing, βinit = nothing,
    σ²yinit = nothing, σ²xinit = nothing, κvinit = nothing, δvinit = nothing,
    ...
    resid = [z[k] - Vf[k] * βf for k in 1:K]
    ...
    for i = 1:niter
        ...
        CQᵦ = cholesky(Qᵦ)
        μᵦ = CQᵦ.U \ (CQᵦ.L \ sum([σ²y[k]^(-1) * Vf[k]' * (z[k]) for k in 1:K]))
    ...
    Dict("v" => δv_samples, "beta" => βf_samples,
